...
def test_ingest_cohort_batch_loads_evidence_directory(tmp_path):
    ...
    for sample_id, output in [("s1", 1000), ("s2", 5)]:
        e = build_evidence(
            sample_id=sample_id, getrpf_version="0.3.0",
            read_count_input=1000, read_count_output=output,
            biological_screen={
                "length_shape": "peaked", "insert_mode": 30,
                "adapter_dimer_fraction": 0.0, "contamination_screened": False,
                "contamination": {}, "duplication_umi_aware": 0.1,
                "verdict": "consistent_with_riboseq", "reasons": ["peaked"],
            },
        )
