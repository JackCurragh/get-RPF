...
def test_no_trim_clean_sample_is_clean_no_trim():
    evidence = build_evidence(
        sample_id="s1", getrpf_version="0.3.0",
        read_count_input=1000, read_count_output=1000,
        biological_screen=_base_screen(),
    )
...
def test_uniform_trim_is_clean_after_terminal_trim():
    ...
    evidence = build_evidence(
        sample_id="s1", getrpf_version="0.3.0",
        read_count_input=1000, read_count_output=950,
        applied_rules=applied, biological_screen=_base_screen(),
    )
...
def test_low_yield_is_excluded_or_held():
    evidence = build_evidence(
        sample_id="s1", getrpf_version="0.3.0",
        read_count_input=1000, read_count_output=10,
        biological_screen=_base_screen(),
    )
...
def test_contamination_inconsistent_is_excluded_or_held():
    ...
    evidence = build_evidence(
        sample_id="s1", getrpf_version="0.3.0",
        read_count_input=1000, read_count_output=900,
        biological_screen=screen,
    )
    assert evidence["release_class"] == "exclude_or_hold"
    assert evidence["warnings"] == []  # no crash on empty warnings
    assert "high_contamination" in screen["reason_codes"]  # machine-readable reason present
...
def test_every_sample_has_evidence_json_and_cohort_is_reproducible(tmp_path):
    evidence_list = [
        build_evidence(
            sample_id="clean1", getrpf_version="0.3.0",
            read_count_input=1000, read_count_output=1000,
            biological_screen=_base_screen(),
        ),
        build_evidence(
            sample_id="held1", getrpf_version="0.3.0",
            read_count_input=1000, read_count_output=5,
            biological_screen=_base_screen(),
            warnings=["extraction yield below 5%"],
        ),
        build_evidence(
            sample_id="review1", getrpf_version="0.3.0",
            read_count_input=1000, read_count_output=900,
            self_consistency={
                "constant_region_ok": None, "umi_region_ok": None,
                "no_residual_cliff": False, "no_overtrim": True,
    ...
    assert held_data["release_class"] in ("exclude_or_hold", "basecaller_artifact_hold", "adapter_dimer_exclude")
    ...
    for e in evidence_list:
        ...
        assert float(by_id[e["sample_id"]]["retained_fraction"]) == e["retained_fraction"]
